...
# Successful write → nudge janus (SIGUSR1) so neon status refreshes
# immediately instead of waiting for its 120s ticker. Write detection is
# by AppleScript verb so read-only scripts (e.g. janus's own fetch_points)
# don't signal — that would loop. Best-effort: a dead/stale pid is ignored.
case "$script" in
    *"set value"*|*"set formula"*)
        tg_pid_file="${HOME}/.cache/janus.pid"
