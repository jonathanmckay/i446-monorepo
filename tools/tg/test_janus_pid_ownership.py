"""Regression tests for the janus SIGUSR1 pid-file registration.

Bug (2026-06-11): a task started in dtd didn't show in janus for ~60s and the
idle alarm kept flashing. toggl_cli sends SIGUSR1 to the pid in
~/.cache/janus.pid after every mutating command, but the file was gone: any
second janus instance's exit cleanup unconditionally unlinked it, deleting the
...
def _load_tui():
    spec = importlib.util.spec_from_file_location("janus_pid", HERE / "janus.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["janus_pid"] = mod
...
def _with_pid_file(tmp_path, monkeypatch):
    mod = _load_tui()
    monkeypatch.setattr(mod, "PID_FILE", tmp_path / "janus.pid")
    return mod
