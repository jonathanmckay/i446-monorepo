...
def _load_tui():
    spec = importlib.util.spec_from_file_location("tg_tui_future", HERE / "tg-tui.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["tg_tui_future"] = mod
...
def test_future_blocks_have_no_ritual_emojis(tmp_path):
    ...
    # Started/in-progress blocks keep their icons.
    assert "巳" in result and "☀️" in result["巳"]
    assert "午" in result, "in-progress block 午 must keep its icons"
