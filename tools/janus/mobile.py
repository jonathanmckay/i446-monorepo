...
def _fetch_events_today(entries: list[dict]) -> list[dict]:
    ...
    tolerance tools/tg/janus.py's own fetch_gcal gives itself."""
    today = _dt.datetime.now(TZ).date()
    day_start = _dt.datetime.combine(today, _dt.time(0, 0), TZ)
    ...
    if gcal_client is not None:
        try:
            raw += gcal_client.list_events(day_start, day_end)
        except Exception as e:
            print("WARN gcal fetch:", e, file=sys.stderr)
    if outlook_client is not None:
        try:
            raw += outlook_client.list_events(day_start, day_end)
        except Exception as e:
            print("WARN outlook fetch:", e, file=sys.stderr)
