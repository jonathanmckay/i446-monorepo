...
// ── LAYOUT CONSTANTS ──
const CW = 158, CH = 78, HGAP = 14, VGAP = 65;
const COMPACT_N = 1, CCW = 138, CCH = 58, CCGAP = 5, PP = 12;
...
function compactPanelWidth(kids) {
  ...
  let maxCols = 0;
  depts.forEach(d => { const n = kids.filter(k => k.d === d).length; maxCols = Math.max(maxCols, Math.min(5, n)); });
